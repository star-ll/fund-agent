...
用户提出"帮我盯着 XX 基金，跌到/涨到 X 元提醒我"、"加一个预警"等需求时。

**步骤：**
1. 如果用户说了"取消"、"删除"、"不用盯了" → 先调 `get_price_alerts` 列出已有预警，确认后调 `delete_price_alert`
2. 如果用户说了"查看"、"有哪些预警" → 调 `get_price_alerts` 展示列表
3. 如果用户说了"检查"、"到了吗"、"触发了吗" → 调 `check_price_alerts`，返回触发结果
4. 新建预警：
   a. 先调 `get_fund_info` 确认基金存在、当前净值
   b. 再调 `set_price_alert` 创建预警，备注可选（如"抄底位置"、"止盈线"）
   c. 告知用户目标价位距当前的距离（X%）

**输出骨架：**
```
> 🔔 已设置预警: [基金名称] [涨破/跌破] [目标净值]
> 📍 当前净值: X / 距目标: ±Y%
> 📋 当前共 N 个活跃预警
```
